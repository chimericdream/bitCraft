...
public class RenderCloud extends TileEntitySpecialRenderer {
    ...
    public void renderTileEntityAt(TileEntity te, double x, double y, double z, float scale) {
        ...
        switch (((TileEntityCloud) te).getConnected()) {
            case 0:
                if (((TileEntityCloud) te).getDirection()) {
                    this.bindTexture(textureRcvNew);
                } else {
                    this.bindTexture(textureXmitNew);
                }
                break;
            case 1:
                if (((TileEntityCloud) te).getDirection()) {
                    this.bindTexture(textureRcvOn);
                } else {
                    this.bindTexture(textureXmitOn);
                }
                break;
            case 2:
                if (((TileEntityCloud) te).getDirection()) {
                    this.bindTexture(textureRcvOff);
                } else {
                    this.bindTexture(textureXmitOff);
                }
                break;
            case 3:
                if (((TileEntityCloud) te).getDirection()) {
                    this.bindTexture(textureRcvBad);
                } else {
                    this.bindTexture(textureXmitBad);
                }
                break;
        }
