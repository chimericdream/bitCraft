...
public class CloudHandler extends WebSocketClient {
    ...
    public void onClose(int statusCode, String reason, boolean remote) {
        ...
        try {
            te.setInputPower(0);
            te.setPower(0);
            Block block = MinecraftServer.getServer()
                                         .worldServerForDimension(te.getDimension())
                                         .getBlock(te.xCoord, te.yCoord, te.zCoord);

            if (block instanceof BlockCloud && !te.getDisabled()) {
                te.setConnected(3);
            }
        } catch (Throwable t) {
    ...
    public void sendPower(int power) {
        te = CloudMod.cloudManager.getAgent(te).getTileEntity();
        try {
            this.send("{\"name\":\"output\",\"args\":{\"device_id\":\"" + te.getDevID() + "\",\"duration_ms\": -1, \"percent\": " + Integer
                    .toString(power) + "}}");
        } catch (Throwable t) {
            System.out.println("wse");
        }
