...
public class TileEntityCloud extends TileEntity {
    ...
    public void setPower(int newPower) {
        power = newPower;
        if (!direction) {
            if (CloudMod.cloudManager.isRegistered(this) && CloudMod.cloudManager.getAgent(this)
                                                                                 .getSocket() != null && !CloudMod.cloudManager
                    .getAgent(this)
                    .getSocket()
                    .isConnecting() && CloudMod.cloudManager.getAgent(this).getSocket().isOpen()) {
    ...
    public void connect() {
        if (!disabled && this.getAuth().length() == 64 && !CloudMod.cloudManager.isRegistered(this)) {
            ...
            CloudMod.playerManager.addPlayerSettings(
                    ownerID,
                    authToken,
                    devID
            );         // Catalog existing settings in DB on startup
        }
    }
}
